...
Watching: rising watch chooses the nearest guard. Q/watch toggles the chosen moving guard's eyes; player BODY does not move or rotate, but time and every guard continue. cycleView changes to next guard only while watching and only matters when multiple guards exist. Q toggles back to the body viewpoint. Unlimited watching has positional risk, not an energy meter. Guard camera yaw comes from actual current patrol facing; body mouse look is ignored while watching. No movement through the possessed view.
...
First30seconds: explicit one-sentence hook and controls, first wing starts in a refuge; explain that normal movement is quiet crouched walking, Shift hurries with extra noise, and Q toggles the guard's view. Show active body vs watcher identity, suspicion bar, refuge/crouch status, collected/total count and one current objective, nearby E prompt and exit objective. A small compass/objective label can point to DISCOVERED relics only; never reveal unknown positions in normal play. Do not drown first-person view in dashboard cards.

Desktop: WASD movement/strafe, mouse look when canvas clicked/pointer locked, ←/→ turn and ↑/↓ forward/back as keyboard-only alternative; movement starts crouched quietly and Shift hold hurries with more noise; Q toggles watch; Tab cycles guard while watching; E interact; R retry from defeat; Esc pause/release pointer lock; Enter start/next/retry; M mute. Pointer lock request only on explicit canvas click, never on title load or touch. Catch unsupported/denied pointer lock and use captured pointer-drag look without blocking keyboard play. Pause/blur/hidden clears input; simulation time freezes. Mouse deltas consumed once per frame; use fixed1/60 simulation. Avoid hold-repeating discrete E/Tab actions.

Touch: explicit visible forward/back and turn-left/right holds, hurry toggle, watch toggle, interact and next-watcher buttons; optional captured drag on the right side of the world looks around. Genuine simultaneous touch controls use independent pointer IDs; no losing a held action when another pointer is released. Targets>=44px, unobscured at390×844 and844×390. Keep view above controls and keyboard help concise.
